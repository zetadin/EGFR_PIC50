...
"""
 This is a prediction only a pipeline for compounds active (pIC50>8) with EGFR.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='Classifies molecules as active or inactive with'
                     'respect to binding to EGFR with pIC50>8',
    ...
    parser.add_argument('-o', dest='output', type=str,
                        default=None,
                        help='Output file for SMILES strings of predicted'
                             ' active compounds. None prints to screen.')
    parser.add_argument('--maxcache', dest='maxcache', type=float,
                        default=512,
                        help='Maximum requested size (in MB) of the in-memory cache'
                             ' for storing features of all the molecules.'
                             ' Increase this if running predictions for a'
    ...
    smis = []
    with open(args.input, 'rt', encoding='ascii') as inpfile:
    ...
    ds = CustomMolDataset(molecules,
                          ...
                          normalize_x = False,      # do not normalize the features,
    ...
    # This will force the CustomMolDataset to normalize descriptors
    # for the new molecules the same way as during training
    ...
    if args.output: # output file specified
        ...
        with open(args.output, "wt", encoding='ascii') as of:
